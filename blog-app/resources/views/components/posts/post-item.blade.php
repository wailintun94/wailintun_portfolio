@props(['post'])
<div>
    <article class="[&:not(:last-child)]:border-b border-gray-100 pb-10 p-4 mb-4 shadow rounded-md">
        <div class="article-body grid grid-cols-12 gap-3 mt-5 items-start">
            <div class="article-thumbnail col-span-4 flex items-center">
                <a wire:navigate href="{{ route('posts.show', $post->slug) }}">
                    <img class="mw-100 mx-auto rounded-xl object-contain h-48 w-96" src="{{ $post->getThumbnailImage() }}"
                        alt="thumbnail">
                </a>
            </div>
            <div class="col-span-8">
                <div class="flex justify-between">
                    <div class="article-meta flex py-1 text-sm items-center">
                        ...
                        <span class="mr-1 text-sm">{{ $post->author->name }}</span>
                        <span class="text-gray-500 text-xs">| {{ $post->publish_at->diffForHumans() }}</span>
                    </div>
                    <div class="flex items-center space-x-4 gap-2">
                        <span class="text-gray-500 text-xs">{{ $post->getReadingTime() }} min read</span>
                    </div>
                </div>
                    ...
                    <div class="flex gap-2">
                        @foreach ($post->categories as $category)
                            <x-badge wire:navigate
                                href="{{ route('posts.index', ['category' => $category->category]) }}"
